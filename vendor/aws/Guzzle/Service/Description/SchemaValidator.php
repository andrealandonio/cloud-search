 ...
 */
class SchemaValidator implements \WP_Cloud_Search\Guzzle\Service\Description\ValidatorInterface
{
     ...
     */
    public function __construct($castIntegerToStringType = \true)
    {
        $this->castIntegerToStringType = $castIntegerToStringType;
    }
    public function validate(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $param, &$value)
    {
        $this->errors = array();
     ...
     */
    protected function recursiveProcess(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $param, &$value, $path = '', $depth = 0)
    {
        ...
        if ($type == 'object') {
            ...
            if (!$valueIsArray && $value instanceof \WP_Cloud_Search\Guzzle\Common\ToArrayInterface) {
                $value = $value->toArray();
            }
            if ($valueIsArray) {
                ...
                $value = array();
                $temporaryValue = $valueIsArray = $traverse = \true;
            }
            if ($traverse) {
                ...
                if ($additional !== \true) {
                    ...
                    if (!empty($diff)) {
                        ...
                        if ($additional instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter) {
                            foreach ($diff as $key) {

...
namespace WP_Cloud_Search\Guzzle\Service\Description;
 ...
 */
class Operation implements \WP_Cloud_Search\Guzzle\Service\Description\OperationInterface
{
    /** @var string Default command class to use when none is specified */
    const DEFAULT_COMMAND_CLASS = 'WP_Cloud_Search\\Guzzle\\Service\\Command\\OperationCommand';
    /** @var array Hashmap of properties that can be specified. Represented as a hash to speed up constructor. */
    protected static $properties = array('name' => \true, 'httpMethod' => \true, 'uri' => \true, 'class' => \true, 'responseClass' => \true, 'responseType' => \true, 'responseNotes' => \true, 'notes' => \true, 'summary' => \true, 'documentationUrl' => \true, 'deprecated' => \true, 'data' => \true, 'parameters' => \true, 'additionalParameters' => \true, 'errorResponses' => \true);
     ...
     */
    public function __construct(array $config = array(), \WP_Cloud_Search\Guzzle\Service\Description\ServiceDescriptionInterface $description = null)
    {
        ...
        if ($this->parameters) {
            foreach ($this->parameters as $name => $param) {
                if ($param instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter) {
                    $param->setName($name)->setParent($this);
                } elseif (\is_array($param)) {
                    $param['name'] = $name;
                    $this->addParam(new \WP_Cloud_Search\Guzzle\Service\Description\Parameter($param, $this->description));
                }
            }
        }
        if ($this->additionalParameters) {
            if ($this->additionalParameters instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter) {
                $this->additionalParameters->setParent($this);
            } elseif (\is_array($this->additionalParameters)) {
                $this->setadditionalParameters(new \WP_Cloud_Search\Guzzle\Service\Description\Parameter($this->additionalParameters, $this->description));
            }
        }
    }
     ...
     * @return self
     */
    public function addParam(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $param)
    {
    ...
    {
        static $types = array(self::TYPE_PRIMITIVE => \true, self::TYPE_CLASS => \true, self::TYPE_MODEL => \true, self::TYPE_DOCUMENTATION => \true);
        if (!isset($types[$responseType])) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('responseType must be one of ' . \implode(', ', \array_keys($types)));
        }

...
use WP_Cloud_Search\Guzzle\Service\AbstractConfigLoader;
use WP_Cloud_Search\Guzzle\Service\Exception\DescriptionBuilderException;
/**
 * Loader for service descriptions
 */
class ServiceDescriptionLoader extends \WP_Cloud_Search\Guzzle\Service\AbstractConfigLoader
{
    protected function build($config, array $options)
    {
        ...
        return new \WP_Cloud_Search\Guzzle\Service\Description\ServiceDescription(array('apiVersion' => isset($config['apiVersion']) ? $config['apiVersion'] : null, 'baseUrl' => isset($config['baseUrl']) ? $config['baseUrl'] : null, 'description' => isset($config['description']) ? $config['description'] : null, 'operations' => $operations, 'models' => isset($config['models']) ? $config['models'] : null) + $config);
    }
    /**
    ...
    protected function resolveExtension($name, array &$op, array &$operations)
    {
        ...
        foreach ((array) $op['extends'] as $extendedCommand) {
            if (empty($operations[$extendedCommand])) {
                throw new \WP_Cloud_Search\Guzzle\Service\Exception\DescriptionBuilderException("{$name} extends missing operation {$extendedCommand}");
            }

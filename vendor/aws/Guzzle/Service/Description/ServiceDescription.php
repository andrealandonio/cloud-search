 ...
 */
class ServiceDescription implements \WP_Cloud_Search\Guzzle\Service\Description\ServiceDescriptionInterface, \WP_Cloud_Search\Guzzle\Common\ToArrayInterface
{
    ...
    public static function factory($config, array $options = array())
    {
        ...
        if (!self::$descriptionLoader) {
            self::$descriptionLoader = new \WP_Cloud_Search\Guzzle\Service\Description\ServiceDescriptionLoader();
        }
    ...
    public function unserialize($json)
    {
        ...
        $this->fromArray(\json_decode($json, \true));
    }
    public function toArray()
    {
        ...
        if (!empty($this->models)) {
            ...
            foreach ($this->models as $id => $model) {
                $result['models'][$id] = $model instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter ? $model->toArray() : $model;
            }
        }
    ...
    public function getOperation($name)
    {
        ...
        if (!$this->operations[$name] instanceof \WP_Cloud_Search\Guzzle\Service\Description\Operation) {
            $this->operations[$name] = new \WP_Cloud_Search\Guzzle\Service\Description\Operation($this->operations[$name], $this);
        }
     ...
     * @return self
     */
    public function addOperation(\WP_Cloud_Search\Guzzle\Service\Description\OperationInterface $operation)
    {
    ...
    public function getModel($id)
    {
        ...
        if (!$this->models[$id] instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter) {
            $this->models[$id] = new \WP_Cloud_Search\Guzzle\Service\Description\Parameter($this->models[$id] + array('name' => $id), $this);
        }
     ...
     * @return self
     */
    public function addModel(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $model)
    {
    ...
    protected function fromArray(array $config)
    {
        ...
        if (isset($config['operations'])) {
            foreach ($config['operations'] as $name => $operation) {
                if (!$operation instanceof \WP_Cloud_Search\Guzzle\Service\Description\Operation && !\is_array($operation)) {
                    throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Invalid operation in service description: ' . \gettype($operation));
                }

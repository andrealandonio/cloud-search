...
use WP_Cloud_Search\Guzzle\Batch\BatchBuilder;
use WP_Cloud_Search\Guzzle\Batch\BatchSizeDivisor;
use WP_Cloud_Search\Guzzle\Batch\BatchClosureTransfer;
 ...
 * @codeCoverageIgnore
 */
class ResourceIteratorApplyBatched extends \WP_Cloud_Search\Guzzle\Common\AbstractHasDispatcher
{
    /** @var callable|array */
    protected $callback;
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Service\Resource\ResourceIteratorInterface $iterator, $callback)
    {
    ...
    public function apply($perBatch = 50)
    {
        ...
        $batch = \WP_Cloud_Search\Guzzle\Batch\BatchBuilder::factory()->createBatchesWith(new \WP_Cloud_Search\Guzzle\Batch\BatchSizeDivisor($perBatch))->transferWith(new \WP_Cloud_Search\Guzzle\Batch\BatchClosureTransfer(function (array $batch) use($that, $callback, &$batches, $it) {
            $batches++;

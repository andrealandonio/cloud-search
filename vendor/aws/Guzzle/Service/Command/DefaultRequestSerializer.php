 ...
 */
class DefaultRequestSerializer implements \WP_Cloud_Search\Guzzle\Service\Command\RequestSerializerInterface
{
    ...
    {
        if (!self::$instance) {
            self::$instance = new self(\WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight::getInstance());
        }
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight $factory)
    {
     ...
     * @return self
     */
    public function addVisitor($location, \WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\Request\RequestVisitorInterface $visitor)
    {
     ...
     */
    protected function prepareAdditionalParameters(\WP_Cloud_Search\Guzzle\Service\Description\OperationInterface $operation, \WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Service\Description\Parameter $additional)
    {
     ...
     */
    protected function createRequest(\WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command)
    {
        $operation = $command->getOperation();
        $client = $command->getClient();
        $options = $command[\WP_Cloud_Search\Guzzle\Service\Command\AbstractCommand::REQUEST_OPTIONS] ?: array();

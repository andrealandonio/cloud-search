...
use WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight;
...
use WP_Cloud_Search\Guzzle\Service\Description\OperationInterface;
use WP_Cloud_Search\Guzzle\Service\Description\Operation;
use WP_Cloud_Search\Guzzle\Service\Exception\ResponseClassException;
 ...
 */
class OperationResponseParser extends \WP_Cloud_Search\Guzzle\Service\Command\DefaultResponseParser
{
    ...
    {
        if (!static::$instance) {
            static::$instance = new static(\WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight::getInstance());
        }
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight $factory, $schemaInModels = \false)
    {
     ...
     * @return self
     */
    public function addVisitor($location, \WP_Cloud_Search\Guzzle\Service\Command\LocationVisitor\Response\ResponseVisitorInterface $visitor)
    {
    ...
    protected function handleParsing(\WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command, \WP_Cloud_Search\Guzzle\Http\Message\Response $response, $contentType)
    {
        ...
        $model = null;
        if ($type == \WP_Cloud_Search\Guzzle\Service\Description\OperationInterface::TYPE_MODEL) {
            $model = $operation->getServiceDescription()->getModel($operation->getResponseClass());
        } elseif ($type == \WP_Cloud_Search\Guzzle\Service\Description\OperationInterface::TYPE_CLASS) {
        ...
        } elseif ($command[\WP_Cloud_Search\Guzzle\Service\Command\AbstractCommand::RESPONSE_PROCESSING] != \WP_Cloud_Search\Guzzle\Service\Command\AbstractCommand::TYPE_MODEL) {
            // Returns a model with no visiting if the command response processing is not model
            return new \WP_Cloud_Search\Guzzle\Service\Resource\Model(parent::handleParsing($command, $response, $contentType));
        } else {
            // Only inject the schema into the model if "schemaInModel" is true
            return new \WP_Cloud_Search\Guzzle\Service\Resource\Model($this->visitResult($model, $command, $response), $this->schemaInModels ? $model : null);
        }
    }
    /**
     ...
     */
    protected function parseClass(\WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command)
    {
        // Emit the operation.parse_class event. If a listener injects a 'result' property, then that will be the result
        $event = new \WP_Cloud_Search\Guzzle\Service\Command\CreateResponseClassEvent(array('command' => $command));
        ...
        if (!\method_exists($className, 'fromCommand')) {
            throw new \WP_Cloud_Search\Guzzle\Service\Exception\ResponseClassException("{$className} must exist and implement a static fromCommand() method");
        }
     ...
     */
    protected function visitResult(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $model, \WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        ...
        if (($additional = $model->getAdditionalProperties()) instanceof \WP_Cloud_Search\Guzzle\Service\Description\Parameter) {
    ...
    protected function visitAdditionalProperties(\WP_Cloud_Search\Guzzle\Service\Description\Parameter $model, \WP_Cloud_Search\Guzzle\Service\Command\CommandInterface $command, \WP_Cloud_Search\Guzzle\Http\Message\Response $response, \WP_Cloud_Search\Guzzle\Service\Description\Parameter $additional, &$result, array &$foundVisitors)
    {

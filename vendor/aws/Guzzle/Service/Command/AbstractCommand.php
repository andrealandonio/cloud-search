...
use WP_Cloud_Search\Guzzle\Service\Description\ValidatorInterface;
use WP_Cloud_Search\Guzzle\Service\Description\SchemaValidator;
use WP_Cloud_Search\Guzzle\Service\Exception\CommandException;
 ...
 */
abstract class AbstractCommand extends \WP_Cloud_Search\Guzzle\Common\Collection implements \WP_Cloud_Search\Guzzle\Service\Command\CommandInterface
{
     ...
     */
    public function __construct($parameters = array(), \WP_Cloud_Search\Guzzle\Service\Description\OperationInterface $operation = null)
    {
        ...
        if (!$headers instanceof \WP_Cloud_Search\Guzzle\Common\Collection) {
            $this[self::HEADERS_OPTION] = new \WP_Cloud_Search\Guzzle\Common\Collection((array) $headers);
        }
    ...
    public function setOnComplete($callable)
    {
        if (!\is_callable($callable)) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('The onComplete function must be callable');
        }
    ...
    {
        if (!$this->client) {
            throw new \WP_Cloud_Search\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be executed.');
        }
    ...
    public function setClient(\WP_Cloud_Search\Guzzle\Service\ClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    {
        if (!$this->request) {
            throw new \WP_Cloud_Search\Guzzle\Service\Exception\CommandException('The command must be prepared before retrieving the request');
        }
        return $this->request;
    }
    ...
    public function prepare()
    {
        if (!$this->isPrepared()) {
            if (!$this->client) {
                throw new \WP_Cloud_Search\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be prepared.');
            }
            ...
            if ($options = $this[\WP_Cloud_Search\Guzzle\Service\Client::CURL_OPTIONS]) {
                $this->request->getCurlOptions()->overwriteWith(\WP_Cloud_Search\Guzzle\Http\Curl\CurlHandle::parseCurlConfig($options));
            }
     ...
     * @return self
     */
    public function setValidator(\WP_Cloud_Search\Guzzle\Service\Description\ValidatorInterface $validator)
    {
    ...
    {
        return new \WP_Cloud_Search\Guzzle\Service\Description\Operation(array('name' => \get_class($this)));
    }
    /**
    ...
    {
        $this->result = $this[self::RESPONSE_PROCESSING] != self::TYPE_RAW ? \WP_Cloud_Search\Guzzle\Service\Command\DefaultResponseParser::getInstance()->parse($this) : $this->request->getResponse();
    }
    /**
    ...
    protected function validate()
    {
        ...
        if (!empty($errors)) {
            $e = new \WP_Cloud_Search\Guzzle\Service\Exception\ValidationException('Validation errors: ' . \implode("\n", $errors));
            $e->setErrors($errors);
            throw $e;
        }
    }
    ...
    {
        if (!$this->validator) {
            $this->validator = \WP_Cloud_Search\Guzzle\Service\Description\SchemaValidator::getInstance();
        }

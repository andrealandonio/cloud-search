 ...
 */
abstract class AbstractConfigLoader implements \WP_Cloud_Search\Guzzle\Service\ConfigLoaderInterface
{
    ...
    public function load($config, array $options = array())
    {
        ...
        if (\is_string($config)) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Unknown type passed to configuration loader: ' . \gettype($config));
        } else {
    ...
    protected function loadFile($filename)
    {
        ...
        switch (\pathinfo($filename, \PATHINFO_EXTENSION)) {
            ...
            case 'json':
                ...
                if ($json === \false) {
                    $err = \error_get_last();
                    throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException("Unable to open {$filename}: " . $err['message']);
                }
                $config = \json_decode($json, \true);
                ...
                if ($error = \json_last_error()) {
                    ...
                    throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException("Error loading JSON data from {$filename}: ({$error}) - {$message}");
                }
                break;
            case 'php':
                if (!\is_readable($filename)) {
                    throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException("Unable to open {$filename} for reading");
                }
                $config = (require $filename);
                if (!\is_array($config)) {
                    throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('PHP files must return an array of configuration data');
                }
                break;
            default:
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Unknown file extension: ' . $filename);
        }
        // Keep track of this file being loaded to prevent infinite recursion
        $this->loadedFiles[$filename] = \true;

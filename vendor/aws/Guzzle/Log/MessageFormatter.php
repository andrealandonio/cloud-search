...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response = null, \WP_Cloud_Search\Guzzle\Http\Curl\CurlHandle $handle = null, array $customData = array())
    {
        $cache = $customData;
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $handle, &$cache) {
            ...
            switch ($matches[1]) {
                ...
                case 'response':
                    ...
                    break;
                case 'req_body':
                    $result = $request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface ? (string) $request->getBody() : '';
                    break;
                case 'res_body':
                    $result = $response ? $response->getBody(\true) : '';
                    break;
                case 'ts':
                    $result = \gmdate('c');
                    break;

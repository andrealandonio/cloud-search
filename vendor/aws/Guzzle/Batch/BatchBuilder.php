...
class BatchBuilder
{
    ...
    protected static $mapping = array('request' => 'WP_Cloud_Search\\Guzzle\\Batch\\BatchRequestTransfer', 'command' => 'WP_Cloud_Search\\Guzzle\\Batch\\BatchCommandTransfer');
    /**
     ...
     * @return BatchBuilder
     */
    public function createBatchesWith(\WP_Cloud_Search\Guzzle\Batch\BatchDivisorInterface $divisorStrategy)
    {
     ...
     * @return BatchBuilder
     */
    public function transferWith(\WP_Cloud_Search\Guzzle\Batch\BatchTransferInterface $transferStrategy)
    {
    ...
    public function build()
    {
        if (!$this->transferStrategy) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('No transfer strategy has been specified');
        }
        if (!$this->divisorStrategy) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('No divisor strategy has been specified');
        }
        $batch = new \WP_Cloud_Search\Guzzle\Batch\Batch($this->transferStrategy, $this->divisorStrategy);
        if ($this->exceptionBuffering) {
            $batch = new \WP_Cloud_Search\Guzzle\Batch\ExceptionBufferingBatch($batch);
        }
        if ($this->afterFlush) {
            $batch = new \WP_Cloud_Search\Guzzle\Batch\NotifyingBatch($batch, $this->afterFlush);
        }
        if ($this->autoFlush) {
            $batch = new \WP_Cloud_Search\Guzzle\Batch\FlushingBatch($batch, $this->autoFlush);
        }
        if ($this->history) {
            $batch = new \WP_Cloud_Search\Guzzle\Batch\HistoryBatch($batch);
        }
        return $batch;
    }
}

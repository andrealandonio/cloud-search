...
class Url
{
    ...
    public static function factory($url)
    {
        ...
        if (\false === ($parts = \parse_url($url))) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Was unable to parse malformed url: ' . $url);
        }
        $parts += $defaults;
        // Convert the query string into a QueryString object
        if ($parts['query'] || 0 !== \strlen($parts['query'])) {
            $parts['query'] = \WP_Cloud_Search\Guzzle\Http\QueryString::fromString($parts['query']);
        }
     ...
     */
    public function __construct($scheme, $host, $username = null, $password = null, $port = null, $path = null, \WP_Cloud_Search\Guzzle\Http\QueryString $query = null, $fragment = null)
    {
        $this->scheme = $scheme;
        $this->host = $host;
        $this->port = $port;
        ...
        if (!$query) {
            $this->query = new \WP_Cloud_Search\Guzzle\Http\QueryString();
        } else {
            $this->setQuery($query);
        }
        $this->setPath($path);
    ...
    public function setQuery($query)
    {
        if (\is_string($query)) {
            ...
            $this->query = new \WP_Cloud_Search\Guzzle\Http\QueryString($output);
        } elseif (\is_array($query)) {
            $this->query = new \WP_Cloud_Search\Guzzle\Http\QueryString($query);
        } elseif ($query instanceof \WP_Cloud_Search\Guzzle\Http\QueryString) {
            $this->query = $query;
        }
        return $this;
    }
    ...
    private function addQuery(\WP_Cloud_Search\Guzzle\Http\QueryString $new, $strictRfc386)
    {
        if (!$strictRfc386) {

 ...
 */
class CachingEntityBody extends \WP_Cloud_Search\Guzzle\Http\AbstractEntityBodyDecorator
{
     ...
     * {@inheritdoc}
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Http\EntityBodyInterface $body)
    {
        $this->remoteStream = $body;
        $this->body = new \WP_Cloud_Search\Guzzle\Http\EntityBody(\fopen('php://temp', 'r+'));
    }
    /**
    ...
    public function seek($offset, $whence = \SEEK_SET)
    {
        if ($whence == \SEEK_SET) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException(__CLASS__ . ' supports only SEEK_SET and SEEK_CUR seek operations');
        }
        // You cannot skip ahead past where you've read from the remote stream
        if ($byte > $this->body->getSize()) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException("Cannot seek to byte {$byte} when the buffered stream only contains {$this->body->getSize()} bytes");
        }
    ...
    {
        throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException(__CLASS__ . ' does not support custom stream rewind functions');
    }

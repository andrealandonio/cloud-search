...
use WP_Cloud_Search\Guzzle\Http\Client;
use WP_Cloud_Search\Guzzle\Http\ClientInterface;
use WP_Cloud_Search\Guzzle\Stream\StreamRequestFactoryInterface;
use WP_Cloud_Search\Guzzle\Stream\PhpStreamRequestFactory;
/**
...
final class StaticClient
{
     ...
     */
    public static function mount($className = 'Guzzle', \WP_Cloud_Search\Guzzle\Http\ClientInterface $client = null)
    {
    ...
    public static function request($method, $url, $options = array())
    {
        ...
        if (!self::$client) {
            self::$client = new \WP_Cloud_Search\Guzzle\Http\Client();
        }
        // @codeCoverageIgnoreEnd
        $request = self::$client->createRequest($method, $url, null, null, $options);
        if (isset($options['stream'])) {
            if ($options['stream'] instanceof \WP_Cloud_Search\Guzzle\Stream\StreamRequestFactoryInterface) {
                return $options['stream']->fromRequest($request);
            } elseif ($options['stream'] == \true) {
                $streamFactory = new \WP_Cloud_Search\Guzzle\Stream\PhpStreamRequestFactory();

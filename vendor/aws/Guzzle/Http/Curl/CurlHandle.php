...
use WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequest;
...
class CurlHandle
{
     ...
     */
    public static function factory(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        $requestCurlOptions = $request->getCurlOptions();
        $mediator = new \WP_Cloud_Search\Guzzle\Http\Curl\RequestMediator($request, $requestCurlOptions->get('emit_io'));
        ...
        if ($requestCurlOptions->get('debug')) {
            ...
            if (\false === $curlOptions[\CURLOPT_STDERR]) {
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('Unable to create a stream for CURLOPT_STDERR');
            }
            // @codeCoverageIgnoreEnd
            $curlOptions[\CURLOPT_VERBOSE] = \true;
        }
        // Specify settings according to the HTTP method
        if ($method == 'GET') {
            $curlOptions[\CURLOPT_HTTPGET] = \true;
        } elseif ($method == 'HEAD') {
            $curlOptions[\CURLOPT_NOBODY] = \true;
            // HEAD requests do not use a write function
            unset($curlOptions[\CURLOPT_WRITEFUNCTION]);
        } elseif (!$request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequest) {
            ...
            if ($request->getBody()) {
                ...
                if (\count($request->getPostFiles())) {
                    ...
                    $postFields = (string) $request->getPostFields()->useUrlEncoding(\true);
                }
                if ($postFields !== \false) {
                    if ($method == 'POST') {
                        unset($curlOptions[\CURLOPT_CUSTOMREQUEST]);
                        $curlOptions[\CURLOPT_POST] = \true;
                    }
    ...
    public function __construct($handle, $options)
    {
        if (!\is_resource($handle)) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Invalid handle provided');
        }
        if (\is_array($options)) {
            $this->options = new \WP_Cloud_Search\Guzzle\Common\Collection($options);
        } elseif ($options instanceof \WP_Cloud_Search\Guzzle\Common\Collection) {
            $this->options = $options;
        } else {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Expected array or Collection');
        }
        $this->handle = $handle;
    }
    /**
    ...
    {
        return \WP_Cloud_Search\Guzzle\Http\Url::factory($this->options->get(\CURLOPT_URL));
    }
    /**
     ...
     */
    public function updateRequestFromTransfer(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($headers) {
            $parsed = \WP_Cloud_Search\Guzzle\Parser\ParserRegistry::getInstance()->getParser('message')->parseRequest($headers);

 ...
 */
class CurlMultiProxy extends \WP_Cloud_Search\Guzzle\Common\AbstractHasDispatcher implements \WP_Cloud_Search\Guzzle\Http\Curl\CurlMultiInterface
{
    ...
    public function __construct($maxHandles = 3, $selectTimeout = 1.0)
    {
        ...
        \class_exists('WP_Cloud_Search\\Guzzle\\Http\\Message\\Response');
        \class_exists('WP_Cloud_Search\\Guzzle\\Http\\Exception\\CurlException');
    }
    ...
    protected function getAvailableHandle()
    {
        ...
        foreach ($this->handles as $h) {
            if (!\in_array($h, $this->groups, \true)) {
                return $h;
            }
        }
        // All are claimed, so create one
        $handle = new \WP_Cloud_Search\Guzzle\Http\Curl\CurlMulti($this->selectTimeout);

...
class RequestMediator
{
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, $emitIo = \false)
    {
    ...
    public function receiveResponseHeader($curl, $header)
    {
        ...
        if (\strpos($header, 'HTTP/') === 0) {
            ...
            if ($code >= 200 && $code < 300) {
                ...
                $body = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory();
            }
            $response = new \WP_Cloud_Search\Guzzle\Http\Message\Response($code, null, $body);
            ...
            $this->request->getResponse()->addHeader(\trim(\substr($header, 0, $pos)), \trim(\substr($header, $pos + 1)));
        }
        return $length;
    }
    /**

...
use WP_Cloud_Search\Guzzle\Parser\UriTemplate\UriTemplateInterface;
use WP_Cloud_Search\Guzzle\Http\Message\RequestInterface;
use WP_Cloud_Search\Guzzle\Http\Message\RequestFactory;
use WP_Cloud_Search\Guzzle\Http\Message\RequestFactoryInterface;
use WP_Cloud_Search\Guzzle\Http\Curl\CurlMultiInterface;
use WP_Cloud_Search\Guzzle\Http\Curl\CurlMultiProxy;
use WP_Cloud_Search\Guzzle\Http\Curl\CurlHandle;
use WP_Cloud_Search\Guzzle\Http\Curl\CurlVersion;
/**
 * HTTP client
 */
class Client extends \WP_Cloud_Search\Guzzle\Common\AbstractHasDispatcher implements \WP_Cloud_Search\Guzzle\Http\ClientInterface
{
    ...
    const DISABLE_REDIRECTS = \WP_Cloud_Search\Guzzle\Http\RedirectPlugin::DISABLE;
    ...
    public function __construct($baseUrl = '', $config = null)
    {
        if (!\extension_loaded('curl')) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('The PHP cURL extension must be installed to use Guzzle.');
            // @codeCoverageIgnoreEnd
        }
        $this->setConfig($config ?: new \WP_Cloud_Search\Guzzle\Common\Collection());
        $this->initSsl();
        $this->setBaseUrl($baseUrl);
        $this->defaultHeaders = new \WP_Cloud_Search\Guzzle\Common\Collection();
        $this->setRequestFactory(\WP_Cloud_Search\Guzzle\Http\Message\RequestFactory::getInstance());
        $this->userAgent = $this->getDefaultUserAgent();
        if (!$this->config[self::DISABLE_REDIRECTS]) {
            $this->addSubscriber(new \WP_Cloud_Search\Guzzle\Http\RedirectPlugin());
        }
    }
    public final function setConfig($config)
    {
        if ($config instanceof \WP_Cloud_Search\Guzzle\Common\Collection) {
            $this->config = $config;
        } elseif (\is_array($config)) {
            $this->config = new \WP_Cloud_Search\Guzzle\Common\Collection($config);
        } else {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Config must be an array or Collection');
        }
        return $this;
    }
    ...
    public final function setSslVerification($certificateAuthority = \true, $verifyPeer = \true, $verifyHost = 2)
    {
        $opts = $this->config[self::CURL_OPTIONS] ?: array();
        if ($certificateAuthority === \true) {
        ...
        } elseif ($verifyPeer !== \true && $verifyPeer !== \false && $verifyPeer !== 1 && $verifyPeer !== 0) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('verifyPeer must be 1, 0 or boolean');
        } elseif ($verifyHost !== 0 && $verifyHost !== 1 && $verifyHost !== 2) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('verifyHost must be 0, 1 or 2');
        } else {
            ...
            if (\is_file($certificateAuthority)) {
                ...
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('Invalid option passed to ' . self::SSL_CERT_AUTHORITY . ': ' . $certificateAuthority);
            }
        }
    ...
    public function createRequest($method = 'GET', $uri = null, $headers = null, $body = null, array $options = array())
    {
        if (!$uri) {
            ...
            if (\strpos($uri, '://')) {
                ...
                $url = \WP_Cloud_Search\Guzzle\Http\Url::factory($this->getBaseUrl())->combine($this->expandTemplate($uri, $templateVars));
            }
        }
        ...
        if (\count($this->defaultHeaders)) {
            if (!$headers) {
                ...
                $headers += $this->defaultHeaders->toArray();
            } elseif ($headers instanceof \WP_Cloud_Search\Guzzle\Common\Collection) {
    ...
    public function getBaseUrl($expand = \true)
    {
    ...
    {
        return 'Guzzle/' . \WP_Cloud_Search\Guzzle\Common\Version::VERSION . ' curl/' . \WP_Cloud_Search\Guzzle\Http\Curl\CurlVersion::getInstance()->get('version') . ' PHP/' . \PHP_VERSION;
    }
    ...
    {
        if (!$requests instanceof \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface) {
        ...
        } catch (\WP_Cloud_Search\Guzzle\Common\Exception\ExceptionCollection $e) {
            throw $e->getFirst();
        }
    }
    /**
     ...
     * @return self
     */
    public function setCurlMulti(\WP_Cloud_Search\Guzzle\Http\Curl\CurlMultiInterface $curlMulti)
    {
    ...
    {
        if (!$this->curlMulti) {
            $this->curlMulti = new \WP_Cloud_Search\Guzzle\Http\Curl\CurlMultiProxy(self::MAX_HANDLES, $this->getConfig('select_timeout') ?: self::DEFAULT_SELECT_TIMEOUT);
        }
        return $this->curlMulti;
    }
    public function setRequestFactory(\WP_Cloud_Search\Guzzle\Http\Message\RequestFactoryInterface $factory)
    {
     ...
     * @return self
     */
    public function setUriTemplate(\WP_Cloud_Search\Guzzle\Parser\UriTemplate\UriTemplateInterface $uriTemplate)
    {
    ...
    {
        if (!$this->uriTemplate) {
            $this->uriTemplate = \WP_Cloud_Search\Guzzle\Parser\ParserRegistry::getInstance()->getParser('uri_template');
        }
     ...
     */
    protected function prepareRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, array $options = array())
    {
        $request->setClient($this)->setEventDispatcher(clone $this->getEventDispatcher());
        if ($curl = $this->config[self::CURL_OPTIONS]) {
            $request->getCurlOptions()->overwriteWith(\WP_Cloud_Search\Guzzle\Http\Curl\CurlHandle::parseCurlConfig($curl));
        }
        if ($params = $this->config[self::REQUEST_PARAMS]) {
            \WP_Cloud_Search\Guzzle\Common\Version::warn('request.params is deprecated. Use request.options to add default request options.');
        ...
        if ($defaults = $this->config[self::REQUEST_OPTIONS]) {
            $this->requestFactory->applyOptions($request, $defaults, \WP_Cloud_Search\Guzzle\Http\Message\RequestFactoryInterface::OPTIONS_AS_DEFAULTS);
        }
        if ($options) {
    ...
    protected function initSsl()
    {
        ...
        if ($authority === \true && \substr(__FILE__, 0, 7) == 'phar://') {
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the request.options array to retrieve default request options');
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the request.options array to specify default request options');
        if ($headers instanceof \WP_Cloud_Search\Guzzle\Common\Collection) {
            $this->defaultHeaders = $headers;
        } elseif (\is_array($headers)) {
            $this->defaultHeaders = new \WP_Cloud_Search\Guzzle\Common\Collection($headers);
        } else {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Headers must be an array or Collection');
        }
        return $this;
    }
    /**
     * @deprecated
     */
    public function preparePharCacert($md5Check = \true)
    {
    ...
    public static function extractPharCacert($pharCacertPath)
    {
        ...
        if (!\file_exists($certFile) || \filesize($certFile) != \filesize($pharCacertPath)) {
            if (!\copy($pharCacertPath, $certFile)) {
                throw new \RuntimeException("Could not copy {$pharCacertPath} to {$certFile}: " . \var_export(\error_get_last(), \true));
            }
        }
        return $certFile;
    }

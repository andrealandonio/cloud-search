...
use WP_Cloud_Search\Guzzle\Http\Mimetypes;
/**
 * Entity body used with an HTTP request or response
 */
class EntityBody extends \WP_Cloud_Search\Guzzle\Stream\Stream implements \WP_Cloud_Search\Guzzle\Http\EntityBodyInterface
{
    ...
    {
        if ($resource instanceof \WP_Cloud_Search\Guzzle\Http\EntityBodyInterface) {
            return $resource;
        }
        ...
        throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Invalid resource type');
    }
    public function setRewindFunction($callable)
    {
        if (!\is_callable($callable)) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Must specify a callable');
        }
    ...
    public function uncompress($filter = 'zlib.inflate')
    {
        ...
        if ($filter == 'zlib.inflate') {
            ...
            if (\stream_get_contents($this->stream, 3, 0) === "\37‹\10") {
                $offsetStart = 10;
            }
        }
    ...
    {
        return $this->getUri() ? \WP_Cloud_Search\Guzzle\Http\Mimetypes::getInstance()->fromFilename($this->getUri()) : null;
    }
     ...
     * @codeCoverageIgnore
     */
    public static function calculateMd5(\WP_Cloud_Search\Guzzle\Http\EntityBodyInterface $body, $rawOutput = \false, $base64Encode = \false)
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__CLASS__ . ' is deprecated. Use getContentMd5()');

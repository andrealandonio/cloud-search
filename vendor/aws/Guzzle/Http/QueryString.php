...
use WP_Cloud_Search\Guzzle\Http\QueryAggregator\QueryAggregatorInterface;
use WP_Cloud_Search\Guzzle\Http\QueryAggregator\PhpAggregator;
/**
 * Query string object to handle managing query string parameters and aggregating those parameters together as a string.
 */
class QueryString extends \WP_Cloud_Search\Guzzle\Common\Collection
{
    ...
    public static function fromString($query)
    {
        ...
        if ($foundDuplicates && !$foundPhpStyle) {
            $q->setAggregator(new \WP_Cloud_Search\Guzzle\Http\QueryAggregator\DuplicateAggregator());
        }
        return $q;
    }
    /**
     ...
     */
    public function setAggregator(\WP_Cloud_Search\Guzzle\Http\QueryAggregator\QueryAggregatorInterface $aggregator = null)
    {
        // Use the default aggregator if none was set
        if (!$aggregator) {
            if (!self::$defaultAggregator) {
                self::$defaultAggregator = new \WP_Cloud_Search\Guzzle\Http\QueryAggregator\PhpAggregator();
            }
    ...
    {
        $this->urlEncode = $encode === \true ? self::RFC_3986 : $encode;
        return $this;
    }
    /**

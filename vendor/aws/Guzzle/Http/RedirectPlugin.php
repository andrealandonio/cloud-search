...
use WP_Cloud_Search\Guzzle\Http\Exception\TooManyRedirectsException;
use WP_Cloud_Search\Guzzle\Http\Exception\CouldNotRewindStreamException;
use WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface;
/**
 * Plugin to implement HTTP redirects. Can redirect like a web browser or using strict RFC 2616 compliance
 */
class RedirectPlugin implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function cleanupRequest(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
     ...
     */
    protected function getOriginalRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        $original = $request;
     ...
     */
    protected function createRedirectRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, $statusCode, $location, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $original)
    {
        ...
        if ($request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface && ($statusCode == 303 || !$strict && $statusCode <= 302)) {
            $redirectRequest = \WP_Cloud_Search\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
        } else {
            $redirectRequest = clone $request;
        }
        $redirectRequest->setIsRedirect(\true);
        // Always use the same response body when redirecting
        $redirectRequest->setResponseBody($request->getResponseBody());
        $location = \WP_Cloud_Search\Guzzle\Http\Url::factory($location);
        // If the location is not absolute, then combine it with the original URL
        if (!$location->isAbsolute()) {
            $originalUrl = $redirectRequest->getUrl(\true);
            // Remove query string parameters and just take what is present on the redirect Location header
            $originalUrl->getQuery()->clear();
            $location = $originalUrl->combine((string) $location, \true);
        }
        ...
        $redirectRequest->getEventDispatcher()->addListener('request.before_send', $func = function ($e) use(&$func, $request, $redirectRequest) {
            ...
            $e['request']->getParams()->set(\WP_Cloud_Search\Guzzle\Http\RedirectPlugin::PARENT_REQUEST, $request);
        });
        // Rewind the entity body of the request if needed
        if ($redirectRequest instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface && $redirectRequest->getBody()) {
            ...
            if ($body->ftell() && !$body->rewind()) {
                throw new \WP_Cloud_Search\Guzzle\Http\Exception\CouldNotRewindStreamException('Unable to rewind the non-seekable entity body of the request after redirecting. cURL probably ' . 'sent part of body before the redirect occurred. Try adding acustom rewind function using on the ' . 'entity body of the request using setRewindFunction().');
            }
        }
     ...
     * @return RequestInterface
     */
    protected function prepareRedirection(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $original, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        ...
        if ($current > $max) {
            ...
            return $this->createRedirectRequest($request, $response->getStatusCode(), \trim($response->getLocation()), $original);
        }
    }
    /**
     ...
     */
    protected function sendRedirectRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $original, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        ...
        if ($redirectRequest = $this->prepareRedirection($original, $request, $response)) {
            ...
            } catch (\WP_Cloud_Search\Guzzle\Http\Exception\BadResponseException $e) {
                $e->getResponse();
                if (!$e->getResponse()) {
                    throw $e;
                }
     ...
     */
    protected function throwTooManyRedirectsException(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $original, $max)
    {
        $original->getEventDispatcher()->addListener('request.complete', $func = function ($e) use(&$func, $original, $max) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Http\Exception\TooManyRedirectsException($str);
        });
    }
}

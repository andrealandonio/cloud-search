...
use WP_Cloud_Search\Guzzle\Common\ToArrayInterface;
 ...
 */
class Response extends \WP_Cloud_Search\Guzzle\Http\Message\AbstractMessage implements \Serializable
{
    /**
    ...
    {
        $data = \WP_Cloud_Search\Guzzle\Parser\ParserRegistry::getInstance()->getParser('message')->parseResponse($message);
        if (!$data) {
            return \false;
        }
    ...
    public function __construct($statusCode, $headers = null, $body = null)
    {
        ...
        $this->body = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($body !== null ? $body : '');
        if ($headers) {
            if (\is_array($headers)) {
                $this->setHeaders($headers);
            } elseif ($headers instanceof \WP_Cloud_Search\Guzzle\Common\ToArrayInterface) {
                $this->setHeaders($headers->toArray());
            } else {
                throw new \WP_Cloud_Search\Guzzle\Http\Exception\BadResponseException('Invalid headers argument received');
            }
        }
    }
    /**
    ...
    {
        $data = \json_decode($serialize, \true);
    ...
    {
        $this->body = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($body);
        return $this;
    }
    /**
    ...
    public function calculateAge()
    {
        ...
        if ($age === null && $this->getDate()) {
            $age = \time() - \strtotime($this->getDate());
        }
    ...
    public function isMethodAllowed($method)
    {
        ...
        if ($allow) {
            foreach (\explode(',', $allow) as $allowable) {
                if (!\strcasecmp(\trim($allowable), $method)) {
                    return \true;
                }
            }
        }
    ...
    public function getMaxAge()
    {
        ...
        if ($this->getHeader('Expires')) {
            return \strtotime($this->getExpires()) - \time();
        }
        return null;
    }
    /**
    ...
    {
        $data = \json_decode((string) $this->body, \true);
        if (\JSON_ERROR_NONE !== \json_last_error()) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('Unable to parse response body into JSON: ' . \json_last_error());
        }
    ...
    public function xml()
    {
        ...
        if ($errorMessage) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('Unable to parse response body into XML: ' . $errorMessage);
        }
        return $xml;
    }
    /**
    ...
    {
        return (int) $this->params->get(\WP_Cloud_Search\Guzzle\Http\RedirectPlugin::REDIRECT_COUNT);
    }
    /**
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the HistoryPlugin.');
        return null;
    }
    /**
     * @deprecated

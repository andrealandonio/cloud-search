...
use WP_Cloud_Search\Guzzle\Http\Exception\BadResponseException;
use WP_Cloud_Search\Guzzle\Http\ClientInterface;
 ...
 */
class Request extends \WP_Cloud_Search\Guzzle\Http\Message\AbstractMessage implements \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface
{
    ...
    public function __construct($method, $url, $headers = array())
    {
        ...
        $this->curlOptions = new \WP_Cloud_Search\Guzzle\Common\Collection();
        $this->setUrl($url);
        if ($headers) {
    ...
    public function __clone()
    {
        ...
        $this->setState(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface::STATE_NEW);
     ...
     */
    public static function onRequestError(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
        $e = \WP_Cloud_Search\Guzzle\Http\Exception\BadResponseException::factory($event['request'], $event['response']);
        $event['request']->setState(self::STATE_ERROR, array('exception' => $e) + $event->toArray());
        throw $e;
    }
    public function setClient(\WP_Cloud_Search\Guzzle\Http\ClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    public function getRawHeaders()
    {
        ...
        return \trim($this->method . ' ' . $this->getResource()) . ' ' . \strtoupper(\str_replace('https', 'http', $this->url->getScheme())) . '/' . $protocolVersion . "\r\n" . \implode("\r\n", $this->getHeaderLines());
    }
    public function setUrl($url)
    {
        if ($url instanceof \WP_Cloud_Search\Guzzle\Http\Url) {
            $this->url = $url;
        } else {
            $this->url = \WP_Cloud_Search\Guzzle\Http\Url::factory($url);
        }
    ...
    {
        if (!$this->client) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException('A client must be set on the request');
        }
    ...
    public function setAuth($user, $password = '', $scheme = \CURLAUTH_BASIC)
    {
        ...
        if (!\is_numeric($scheme)) {
            ...
            if (!isset($authMap[$scheme])) {
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException($scheme . ' is not a valid authentication type');
            }
    ...
    public function startResponse(\WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
    ...
    public function setResponse(\WP_Cloud_Search\Guzzle\Http\Message\Response $response, $queued = \false)
    {
    ...
    public function setResponseBody($body)
    {
        ...
        if (\is_string($body)) {
            ...
            if (!($body = \fopen($body, 'w+'))) {
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Could not open ' . $body . ' for writing');
            }
            // @codeCoverageIgnoreEnd
        }
        $this->responseBody = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($body);
        return $this;
    }
    public function getResponseBody()
    {
        if ($this->responseBody === null) {
            $this->responseBody = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory()->setCustomData('default', \true);
        }
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use $request->getResponseBody()->isRepeatable()');
        return !$this->responseBody ? \true : $this->responseBody->isRepeatable();
    }
    public function getCookies()
    {
        if ($cookie = $this->getHeader('Cookie')) {
            $data = \WP_Cloud_Search\Guzzle\Parser\ParserRegistry::getInstance()->getParser('cookie')->parseCookie($cookie);
            return $data['cookies'];
        }
        return array();
    }
    ...
    {
        if (!$this->eventDispatcher) {
            $this->setEventDispatcher(new \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventDispatcher());
        }
    ...
    protected function processResponse(array $context = array())
    {
        if (!$this->response) {
            ...
            $e = new \WP_Cloud_Search\Guzzle\Http\Exception\RequestException('Error completing request');
            $e->setRequest($this);
            throw $e;
        }
        ...
        if ($this->state == \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface::STATE_COMPLETE) {
            ...
            if ($this->response->isError()) {
                $event = new \WP_Cloud_Search\Guzzle\Common\Event($this->getEventArray());
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use Guzzle\\Plugin\\Cache\\DefaultCanCacheStrategy.');
        if (\class_exists('WP_Cloud_Search\\Guzzle\\Plugin\\Cache\\DefaultCanCacheStrategy')) {
            $canCache = new \WP_Cloud_Search\Guzzle\Plugin\Cache\DefaultCanCacheStrategy();
    ...
    {
        \WP_Cloud_Search\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the HistoryPlugin to track this.');

 ...
 */
class EntityEnclosingRequest extends \WP_Cloud_Search\Guzzle\Http\Message\Request implements \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface
{
    ...
    {
        $this->postFields = new \WP_Cloud_Search\Guzzle\Http\QueryString();
    ...
    public function setBody($body, $contentType = null)
    {
        ...
        if ($size !== null && $size !== \false) {
            ...
            if ('1.1' == $this->protocolVersion) {
                ...
                throw new \WP_Cloud_Search\Guzzle\Http\Exception\RequestException('Cannot determine Content-Length and cannot use chunked Transfer-Encoding when using HTTP/1.0');
            }
        }
        return $this;
    }
    ...
    {
        $this->getParams()->set(\WP_Cloud_Search\Guzzle\Http\RedirectPlugin::STRICT_REDIRECTS, $strict);
        if ($maxRedirects == 0) {
            $this->getParams()->set(\WP_Cloud_Search\Guzzle\Http\RedirectPlugin::DISABLE, \true);
        } else {
            $this->getParams()->set(\WP_Cloud_Search\Guzzle\Http\RedirectPlugin::MAX_REDIRECTS, $maxRedirects);
        }
        return $this;
    }
    ...
    {
        $data = null;
        if ($field instanceof \WP_Cloud_Search\Guzzle\Http\Message\PostFileInterface) {
            $data = $field;
            ...
            throw new \WP_Cloud_Search\Guzzle\Http\Exception\RequestException('The path to a file must be a string');
        } elseif (!empty($filename)) {
            // Adding an empty file will cause cURL to error out
            $data = new \WP_Cloud_Search\Guzzle\Http\Message\PostFile($field, $filename, $contentType, $postname);
        }
        if ($data) {
    ...
    public function addPostFiles(array $files)
    {
        foreach ($files as $key => $file) {
            if ($file instanceof \WP_Cloud_Search\Guzzle\Http\Message\PostFileInterface) {
                ...
                throw new \WP_Cloud_Search\Guzzle\Http\Exception\RequestException('File must be a string or instance of PostFileInterface');
            }
        }
        return $this;
    }

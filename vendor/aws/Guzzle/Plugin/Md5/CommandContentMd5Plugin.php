...
namespace WP_Cloud_Search\Guzzle\Plugin\Md5;
 ...
 */
class CommandContentMd5Plugin implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    public function onCommandBeforeSend(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
        ...
        if ($request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface && $request->getBody() && $command->getOperation()->hasParam($this->contentMd5Param)) {
            // Check if an MD5 checksum value should be passed along to the request
            if ($command[$this->contentMd5Param] === \true) {
                if (\false !== ($md5 = $request->getBody()->getContentMd5(\true, \true))) {
                    $request->setHeader('Content-MD5', $md5);
                }
            }
        }
        // Check if MD5 validation should be used with the response
        if ($command[$this->validateMd5Param] === \true) {
            $request->addSubscriber(new \WP_Cloud_Search\Guzzle\Plugin\Md5\Md5ValidatorPlugin(\true, \false));
        }
    }
}

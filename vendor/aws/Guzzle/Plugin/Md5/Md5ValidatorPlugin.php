 ...
 */
class Md5ValidatorPlugin implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function __construct($contentEncoded = \true, $contentLengthCutoff = \false)
    {
     ...
     */
    public function onRequestComplete(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
        ...
        if ($contentMd5 !== $hash) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\UnexpectedValueException("The response entity body may have been modified over the wire.  The Content-MD5 " . "received ({$contentMd5}) did not match the calculated MD5 hash ({$hash}).");
        }
    }
}

 ...
 */
class DefaultRevalidation implements \WP_Cloud_Search\Guzzle\Plugin\Cache\RevalidationInterface
{
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Plugin\Cache\CacheStorageInterface $cache, \WP_Cloud_Search\Guzzle\Plugin\Cache\CanCacheStrategyInterface $canCache = null)
    {
        $this->storage = $cache;
        $this->canCache = $canCache ?: new \WP_Cloud_Search\Guzzle\Plugin\Cache\DefaultCanCacheStrategy();
    }
    ...
    {
        if ($request->getMethod() != \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface::GET) {
            return \false;
        }
     ...
     */
    protected function handleBadResponse(\WP_Cloud_Search\Guzzle\Http\Exception\BadResponseException $e)
    {
     ...
     */
    protected function createRevalidationRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        ...
        foreach ($dispatcher->getListeners() as $eventName => $listeners) {
            foreach ($listeners as $listener) {
                if (\is_array($listener) && $listener[0] instanceof \WP_Cloud_Search\Guzzle\Plugin\Cache\CachePlugin) {
     ...
     */
    protected function handle200Response(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $validateResponse)
    {
     ...
     * @return bool Returns true if valid, false if invalid
     */
    protected function handle304Response(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $validateResponse, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {

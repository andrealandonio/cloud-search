...
use WP_Cloud_Search\Guzzle\Http\Message\MessageInterface;
use WP_Cloud_Search\Guzzle\Http\Message\Request;
use WP_Cloud_Search\Guzzle\Http\Message\RequestInterface;
use WP_Cloud_Search\Guzzle\Http\Message\Response;
/**
 * Default cache storage implementation
 */
class DefaultCacheStorage implements \WP_Cloud_Search\Guzzle\Plugin\Cache\CacheStorageInterface
{
    /** @var string */
    protected $keyPrefix;
    ...
    {
        $this->cache = \WP_Cloud_Search\Guzzle\Cache\CacheAdapterFactory::fromCache($cache);
    ...
    public function purge($url)
    {
        foreach (array('GET', 'HEAD', 'POST', 'PUT', 'DELETE') as $method) {
            $this->delete(new \WP_Cloud_Search\Guzzle\Http\Message\Request($method, $url));
        }
    }
    public function fetch(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($match[4] < \time()) {
            ...
            $response = new \WP_Cloud_Search\Guzzle\Http\Message\Response($match[2], $match[1]);
            if ($match[3]) {
     ...
     * @return string
     */
    protected function getCacheKey(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($filter = $request->getParams()->get('cache.key_filter')) {
            $url = $request->getUrl(\true);
            foreach (\explode(',', $filter) as $remove) {
                $url->getQuery()->remove(\trim($remove));
            }
        } else {
            $url = $request->getUrl();
        }
     ...
     * @return string
     */
    protected function getBodyKey($url, \WP_Cloud_Search\Guzzle\Http\EntityBodyInterface $body)
    {
     ...
     * @return array
     */
    private function persistHeaders(\WP_Cloud_Search\Guzzle\Http\Message\MessageInterface $message)
    {
        // Headers are excluded from the caching (see RFC 2616:13.5.1)
        static $noCache = array('age' => \true, 'connection' => \true, 'keep-alive' => \true, 'proxy-authenticate' => \true, 'proxy-authorization' => \true, 'te' => \true, 'trailers' => \true, 'transfer-encoding' => \true, 'upgrade' => \true, 'set-cookie' => \true, 'set-cookie2' => \true);

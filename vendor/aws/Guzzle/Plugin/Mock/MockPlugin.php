...
namespace WP_Cloud_Search\Guzzle\Plugin\Mock;
 ...
 */
class MockPlugin extends \WP_Cloud_Search\Guzzle\Common\AbstractHasDispatcher implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface, \Countable
{
    ...
    {
        if (!\file_exists($path)) {
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Unable to open mock file: ' . $path);
        }
        return \WP_Cloud_Search\Guzzle\Http\Message\Response::fromMessage(\file_get_contents($path));
    }
    /**
    ...
    {
        if (!$response instanceof \WP_Cloud_Search\Guzzle\Http\Message\Response) {
            if (!\is_string($response)) {
                throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('Invalid response');
            }
     ...
     * @return MockPlugin
     */
    public function addException(\WP_Cloud_Search\Guzzle\Http\Exception\CurlException $e)
    {
        $this->queue[] = $e;
        return $this;
    }
     ...
     */
    public function dequeue(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        $this->dispatch('mock.request', array('plugin' => $this, 'request' => $request));
        $item = \array_shift($this->queue);
        if ($item instanceof \WP_Cloud_Search\Guzzle\Http\Message\Response) {
            if ($this->readBodies && $request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface) {
                $request->getEventDispatcher()->addListener('request.sent', $f = function (\WP_Cloud_Search\Guzzle\Common\Event $event) use(&$f) {
        ...
        } elseif ($item instanceof \WP_Cloud_Search\Guzzle\Http\Exception\CurlException) {
            // Emulates exceptions encountered while transferring requests
            $item->setRequest($request);
            $state = $request->setState(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface::STATE_ERROR, array('exception' => $item));

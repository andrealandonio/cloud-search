...
use WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie;
...
use WP_Cloud_Search\Guzzle\Plugin\Cookie\Exception\InvalidCookieException;
/**
 * Cookie cookieJar that stores cookies an an array
 */
class ArrayCookieJar implements \WP_Cloud_Search\Guzzle\Plugin\Cookie\CookieJar\CookieJarInterface, \Serializable
{
    ...
    public function remove($domain = null, $path = null, $name = null)
    {
        ...
        $this->cookies = \array_filter($this->cookies, function (\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie) use($cookies) {
            return !\in_array($cookie, $cookies, \true);
        });
        return $this;
    }
    public function removeTemporary()
    {
        $this->cookies = \array_filter($this->cookies, function (\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie) {
    ...
    {
        $currentTime = \time();
        $this->cookies = \array_filter($this->cookies, function (\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie) use($currentTime) {
            return !$cookie->getExpires() || $currentTime < $cookie->getExpires();
        });
        return $this;
    }
    public function all($domain = null, $path = null, $name = null, $skipDiscardable = \false, $skipExpired = \true)
    {
        return \array_values(\array_filter($this->cookies, function (\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie) use($domain, $path, $name, $skipDiscardable, $skipExpired) {
    ...
    public function add(\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie)
    {
        // Only allow cookies with set and valid domain, name, value
        $result = $cookie->validate();
        if ($result !== \true) {
            if ($this->strictMode) {
                throw new \WP_Cloud_Search\Guzzle\Plugin\Cookie\Exception\InvalidCookieException($result);
            } else {
    ...
    public function serialize()
    {
        ...
        return \json_encode(\array_map(function (\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie) {
            return $cookie->toArray();
        }, $this->all(null, null, null, \true, \true)));
    }
    /**
    ...
    public function unserialize($data)
    {
        ...
        if (empty($data)) {
            ...
            $this->cookies = \array_map(function (array $cookie) {
                return new \WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie($cookie);
            }, $data);
        }
    }
    /**
    ...
    public function addCookiesFromResponse(\WP_Cloud_Search\Guzzle\Http\Message\Response $response, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request = null)
    {
        if ($cookieHeader = $response->getHeader('Set-Cookie')) {
            $parser = \WP_Cloud_Search\Guzzle\Parser\ParserRegistry::getInstance()->getParser('cookie');
            foreach ($cookieHeader as $cookie) {
                if ($parsed = $request ? $parser->parseCookie($cookie, $request->getHost(), $request->getPath()) : $parser->parseCookie($cookie)) {
                    ...
                    foreach ($parsed['cookies'] as $key => $value) {
                        ...
                        $row['value'] = $value;
                        unset($row['cookies']);
                        $this->add(new \WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie($row));
                    }
                }
            }
        }
    }
    public function getMatchingCookies(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     */
    private function removeCookieIfEmpty(\WP_Cloud_Search\Guzzle\Plugin\Cookie\Cookie $cookie)
    {

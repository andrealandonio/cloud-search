...
namespace WP_Cloud_Search\Guzzle\Plugin\Log;

use WP_Cloud_Search\Guzzle\Common\Event;
use WP_Cloud_Search\Guzzle\Log\LogAdapterInterface;
use WP_Cloud_Search\Guzzle\Log\MessageFormatter;
 ...
 */
class LogPlugin implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function __construct(\WP_Cloud_Search\Guzzle\Log\LogAdapterInterface $logAdapter, $formatter = null, $wireBodies = \false)
    {
        $this->logAdapter = $logAdapter;
        $this->formatter = $formatter instanceof \WP_Cloud_Search\Guzzle\Log\MessageFormatter ? $formatter : new \WP_Cloud_Search\Guzzle\Log\MessageFormatter($formatter);
     ...
     * @return self
     */
    public static function getDebugPlugin($wireBodies = \true, $stream = null)
    {
        if ($stream === null) {
            if (\defined('STDERR')) {
                $stream = \STDERR;
        ...
        return new self(new \WP_Cloud_Search\Guzzle\Log\ClosureLogAdapter(function ($m) use($stream) {
     ...
     * @param Event $event
     */
    public function onCurlRead(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
     ...
     * @param Event $event
     */
    public function onCurlWrite(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
    ...
    public function onRequestBeforeSend(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
        if ($this->wireBodies) {
            ...
            $request->getCurlOptions()->set('emit_io', \true);
            // We need to make special handling for content wiring and non-repeatable streams.
            if ($request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface && $request->getBody() && (!$request->getBody()->isSeekable() || !$request->getBody()->isReadable())) {
                // The body of the request cannot be recalled so logging the body will require us to buffer it
                $request->getParams()->set('request_wire', \WP_Cloud_Search\Guzzle\Http\EntityBody::factory());
            }
            if (!$request->getResponseBody()->isRepeatable()) {
                ...
                $request->getParams()->set('response_wire', \WP_Cloud_Search\Guzzle\Http\EntityBody::factory());
            }
        }
    }
    /**

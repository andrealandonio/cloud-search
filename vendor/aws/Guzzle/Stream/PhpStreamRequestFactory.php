...
namespace WP_Cloud_Search\Guzzle\Stream;
 ...
 */
class PhpStreamRequestFactory implements \WP_Cloud_Search\Guzzle\Stream\StreamRequestFactoryInterface
{
     ...
     */
    public function fromRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, $context = array(), array $params = array())
    {
        if (\is_resource($context)) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\InvalidArgumentException('$context must be an array or resource');
        }
     ...
     */
    protected function addDefaultContextOptions(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        $this->setContextValue('http', 'ignore_errors', \true);
    }
    /**
     ...
     */
    protected function setUrl(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        $this->url = $request->getUrl(\true);
     ...
     * @param RequestInterface $request Request
     */
    protected function addSslOptions(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        if ($request->getCurlOptions()->get(\CURLOPT_SSL_VERIFYPEER)) {
            $this->setContextValue('ssl', 'verify_peer', \true, \true);
            if ($cafile = $request->getCurlOptions()->get(\CURLOPT_CAINFO)) {
                $this->setContextValue('ssl', 'cafile', $cafile, \true);
            }
        } else {
            $this->setContextValue('ssl', 'verify_peer', \false, \true);
        }
    }
    /**
     ...
     */
    protected function addBodyOptions(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if (\count($request->getPostFields())) {
            $this->setContextValue('http', 'content', (string) $request->getPostFields(), \true);
        } elseif ($request->getBody()) {
            $this->setContextValue('http', 'content', (string) $request->getBody(), \true);
        }
        ...
        if (isset($this->contextOptions['http']['content'])) {
            ...
            $this->setContextValue('http', 'header', $headers, \true);
        }
    }
    /**
     ...
     * @param RequestInterface $request Request
     */
    protected function addProxyOptions(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     */
    protected function processResponseHeaders(\WP_Cloud_Search\Guzzle\Stream\StreamInterface $stream)
    {
        // Set the size on the stream if it was returned in the response
        foreach ($this->lastResponseHeaders as $header) {
            if (\stripos($header, 'Content-Length:') === 0) {
                $stream->setSize(\trim(\substr($header, 15)));
            }
        }
    }
    /**
    ...
    protected function createResource($callback)
    {
        ...
        if (!$resource) {
            ...
            throw new \WP_Cloud_Search\Guzzle\Common\Exception\RuntimeException(\trim($message));
        }
        return $resource;
    }
}

...
use WP_Cloud_Search\PhpConsole\Connector;
use WP_Cloud_Search\PhpConsole\Handler;
use WP_Cloud_Search\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WP_Cloud_Search\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \WP_Cloud_Search\PhpConsole\Connector $connector = null, $level = \WP_Cloud_Search\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('WP_Cloud_Search\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\WP_Cloud_Search\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WP_Cloud_Search\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WP_Cloud_Search\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WP_Cloud_Search\PhpConsole\Helper::isRegistered()) {
            \WP_Cloud_Search\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WP_Cloud_Search\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \WP_Cloud_Search\Monolog\Logger::NOTICE) {
    ...
    {
        return new \WP_Cloud_Search\Monolog\Formatter\LineFormatter('%message%');
    }
}

...
use WP_Cloud_Search\Aws\ClientSideMonitoring\ConfigurationProvider;
use WP_Cloud_Search\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use WP_Cloud_Search\Aws\Signature\SignatureProvider;
use WP_Cloud_Search\GuzzleHttp\Psr7\Uri;
/**
...
class AwsClient implements AwsClientInterface
{
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'WP_Cloud_Search\\Aws\\Exception\\AwsException'];
        }
        $service = \substr($klass, \strrpos($klass, '\\') + 1, -6);
        return [\strtolower($service), "WP_Cloud_Search\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    ...
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \WP_Cloud_Search\Aws\load_compiled_json($file);
    ...
    {
        $aliases = \WP_Cloud_Search\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');

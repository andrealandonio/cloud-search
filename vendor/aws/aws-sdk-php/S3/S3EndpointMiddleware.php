...
use WP_Cloud_Search\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \WP_Cloud_Search\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \WP_Cloud_Search\Aws\strip_fips_pseudo_regions($this->region);

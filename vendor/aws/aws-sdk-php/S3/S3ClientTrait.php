...
use WP_Cloud_Search\Aws\S3\Exception\PermanentRedirectException;
use WP_Cloud_Search\Aws\S3\Exception\S3Exception;
use WP_Cloud_Search\GuzzleHttp\Promise\PromiseInterface;
use WP_Cloud_Search\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    public function deleteMatchingObjectsAsync($bucket, $prefix = '', $regex = '', array $options = [])
    {
        ...
        if ($regex) {
            $iter = \WP_Cloud_Search\Aws\filter($iter, function ($c) use($regex) {

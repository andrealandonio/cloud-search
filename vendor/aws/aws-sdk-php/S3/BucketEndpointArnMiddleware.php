...
use WP_Cloud_Search\Aws\Arn\ArnParser;
use WP_Cloud_Search\Aws\Arn\ObjectLambdaAccessPointArn;
...
use WP_Cloud_Search\Aws\Arn\S3\OutpostsAccessPointArn;
use WP_Cloud_Search\Aws\Arn\S3\MultiRegionAccessPointArn;
use WP_Cloud_Search\Aws\Arn\S3\OutpostsArnInterface;
...
use WP_Cloud_Search\Aws\Exception\UnresolvedEndpointException;
use WP_Cloud_Search\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(BaseAccessPointArn $arn, RequestInterface $req)
    {
        ...
        $region = \WP_Cloud_Search\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!$clientPart->isRegionMatch($this->region, 's3')) {
                $clientPart = $this->partitionProvider->getPartition(\WP_Cloud_Search\Aws\strip_fips_pseudo_regions($this->region), 's3');
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {

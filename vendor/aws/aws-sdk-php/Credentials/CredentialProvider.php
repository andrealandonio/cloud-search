...
namespace WP_Cloud_Search\Aws\Credentials;

use WP_Cloud_Search\Aws;
use WP_Cloud_Search\Aws\Api\DateTimeResult;
use WP_Cloud_Search\Aws\CacheInterface;
use WP_Cloud_Search\Aws\Exception\CredentialsException;
use WP_Cloud_Search\Aws\Sts\StsClient;
use WP_Cloud_Search\GuzzleHttp\Promise;
/**
...
class CredentialProvider
{
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WP_Cloud_Search\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \WP_Cloud_Search\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \WP_Cloud_Search\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \WP_Cloud_Search\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \WP_Cloud_Search\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);

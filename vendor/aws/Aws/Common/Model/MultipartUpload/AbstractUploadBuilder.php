    ...
    public function setSource($source)
    {
        ...
        if (\is_string($source)) {
            if (!\file_exists($source)) {
                throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException("File does not exist: {$source}");
            }
            // Clear the cache so that we send accurate file sizes
            \clearstatcache(\true, $source);
            $source = \fopen($source, 'r');
        }
        $this->source = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($source);
        if ($this->source->isSeekable() && $this->source->getSize() == 0) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException('Empty body provided to upload builder');
        }
        return $this;
    }
    /**

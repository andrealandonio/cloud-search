 ...
 */
namespace WP_Cloud_Search\Aws\Common\Hash;

use WP_Cloud_Search\Aws\Common\Enum\Size;
use WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException;
use WP_Cloud_Search\Aws\Common\Exception\LogicException;
use WP_Cloud_Search\Guzzle\Http\EntityBody;
/**
 * Encapsulates the creation of a tree hash from streamed chunks of data
 */
class TreeHash implements \WP_Cloud_Search\Aws\Common\Hash\ChunkHashInterface
{
    /**
    ...
    public static function fromContent($content, $algorithm = self::DEFAULT_ALGORITHM)
    {
        ...
        $content = \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($content);
        while ($data = $content->read(\WP_Cloud_Search\Aws\Common\Enum\Size::MB)) {
    ...
    {
        \WP_Cloud_Search\Aws\Common\Hash\HashUtils::validateAlgorithm($algorithm);
    ...
    public function addData($data)
    {
        ...
        if ($this->hash) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\LogicException('You may not add more data to a finalized tree hash.');
        }
        // Make sure that only 1MB chunks or smaller get passed in
        if (\strlen($data) > \WP_Cloud_Search\Aws\Common\Enum\Size::MB) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException('The chunk of data added is too large for tree hashing.');
        }
    ...
    public function addChecksum($checksum, $inBinaryForm = \false)
    {
        // Error if hash is already calculated
        if ($this->hash) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\LogicException('You may not add more checksums to a finalized tree hash.');
        }
        // Convert the checksum to binary form if necessary
        $this->checksums[] = $inBinaryForm ? $checksum : \WP_Cloud_Search\Aws\Common\Hash\HashUtils::hexToBin($checksum);
        return $this;
    }
    /**
     * {@inheritdoc}
    ...
    public function getHash($returnBinaryForm = \false)
    {
        if (!$this->hash) {
            ...
            $this->hash = \WP_Cloud_Search\Aws\Common\Hash\HashUtils::binToHex($this->hashRaw);
        }

...
use WP_Cloud_Search\Guzzle\Service\Command\AbstractCommand as Command;
 ...
 */
class UploadBodyListener implements \WP_Cloud_Search\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    /**
     ...
     */
    public function onCommandBeforePrepare(\WP_Cloud_Search\Guzzle\Common\Event $event)
    {
        /** @var Command $command */
        ...
        if (\in_array($command->getName(), $this->commands)) {
            ...
            if (null !== $body) {
                ...
                $command->set($this->bodyParameter, \WP_Cloud_Search\Guzzle\Http\EntityBody::factory($body));
            } else {
                throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException("You must specify a non-null value for the {$this->bodyParameter} or {$this->sourceParameter} parameters.");
            }
        }
    }
}

...
use WP_Cloud_Search\Aws\Common\Exception\TransferException;
...
use WP_Cloud_Search\Aws\Common\Signature\SignatureListener;
use WP_Cloud_Search\Aws\Common\Waiter\WaiterClassFactory;
use WP_Cloud_Search\Aws\Common\Waiter\CompositeWaiterFactory;
use WP_Cloud_Search\Aws\Common\Waiter\WaiterFactoryInterface;
use WP_Cloud_Search\Aws\Common\Waiter\WaiterConfigFactory;
use WP_Cloud_Search\Guzzle\Common\Collection;
use WP_Cloud_Search\Guzzle\Http\Exception\CurlException;
use WP_Cloud_Search\Guzzle\Http\QueryAggregator\DuplicateAggregator;
use WP_Cloud_Search\Guzzle\Service\Client;
use WP_Cloud_Search\Guzzle\Service\Description\ServiceDescriptionInterface;
/**
 * Abstract AWS client
 */
abstract class AbstractClient extends \WP_Cloud_Search\Guzzle\Service\Client implements \WP_Cloud_Search\Aws\Common\Client\AwsClientInterface
{
    ...
    {
        return \array_merge(\WP_Cloud_Search\Guzzle\Service\Client::getAllEvents(), array('client.region_changed', 'client.credentials_changed'));
    }
    /**
     ...
     */
    public function __construct(\WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials, \WP_Cloud_Search\Aws\Common\Signature\SignatureInterface $signature, \WP_Cloud_Search\Guzzle\Common\Collection $config)
    {
        // Bootstrap with Guzzle
        parent::__construct($config->get(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::BASE_URL), $config);
        $this->credentials = $credentials;
        $this->signature = $signature;
        $this->aggregator = new \WP_Cloud_Search\Guzzle\Http\QueryAggregator\DuplicateAggregator();
        // Make sure the user agent is prefixed by the SDK version
        $this->setUserAgent('aws-sdk-php2/' . \WP_Cloud_Search\Aws\Common\Aws::VERSION, \true);
        // Add the event listener so that requests are signed before they are sent
        $dispatcher = $this->getEventDispatcher();
        $dispatcher->addSubscriber(new \WP_Cloud_Search\Aws\Common\Signature\SignatureListener($credentials, $signature));
        if ($backoff = $config->get(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::BACKOFF)) {
     ...
     */
    public static function getEndpoint(\WP_Cloud_Search\Guzzle\Service\Description\ServiceDescriptionInterface $description, $region, $scheme)
    {
        try {
            $service = $description->getData('endpointPrefix');
            $provider = \WP_Cloud_Search\Aws\Common\RulesEndpointProvider::fromDefaults();
            $result = $provider(array('service' => $service, 'region' => $region, 'scheme' => $scheme));
            return $result['endpoint'];
        } catch (\InvalidArgumentException $e) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException($e->getMessage(), 0, $e);
        }
    }
    ...
    public function setCredentials(\WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
    ...
    {
        return $this->getConfig(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::REGION);
    }
    public function setRegion($region)
    {
        $config = $this->getConfig();
        $formerRegion = $config->get(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::REGION);
        ...
        if (!$global || $this->serviceDescription->getData('namespace') === 'S3') {
            $endpoint = \call_user_func($provider, array('scheme' => $config->get(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::SCHEME), 'region' => $region, 'service' => $config->get(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::SERVICE)));
            $this->setBaseUrl($endpoint['endpoint']);
            $config->set(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::BASE_URL, $endpoint['endpoint']);
            $config->set(\WP_Cloud_Search\Aws\Common\Enum\ClientOptions::REGION, $region);
    ...
    public function setWaiterFactory(\WP_Cloud_Search\Aws\Common\Waiter\WaiterFactoryInterface $waiterFactory)
    {
    ...
    public function getWaiterFactory()
    {
        if (!$this->waiterFactory) {
            ...
            $this->waiterFactory = new \WP_Cloud_Search\Aws\Common\Waiter\CompositeWaiterFactory(array(new \WP_Cloud_Search\Aws\Common\Waiter\WaiterClassFactory(\substr($clientClass, 0, \strrpos($clientClass, '\\')) . '\\Waiter')));
            if ($this->getDescription()) {
                $waiterConfig = $this->getDescription()->getData('waiters') ?: array();
                $this->waiterFactory->addFactory(new \WP_Cloud_Search\Aws\Common\Waiter\WaiterConfigFactory($waiterConfig));
            }
        }
    ...
    public function send($requests)
    {
        try {
            ...
            $wrapped = new \WP_Cloud_Search\Aws\Common\Exception\TransferException($e->getMessage(), null, $e);

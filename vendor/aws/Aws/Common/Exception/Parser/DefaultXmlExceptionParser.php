 ...
 */
namespace WP_Cloud_Search\Aws\Common\Exception\Parser;
 ...
 */
class DefaultXmlExceptionParser implements \WP_Cloud_Search\Aws\Common\Exception\Parser\ExceptionParserInterface
{
    public function parse(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        $data = array('code' => null, 'message' => null, 'type' => $response->isClientError() ? 'client' : 'server', 'request_id' => null, 'parsed' => null);
        $body = $response->getBody(\true);
        if (!$body) {
     ...
     */
    protected function parseHeaders(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response, array &$data)
    {

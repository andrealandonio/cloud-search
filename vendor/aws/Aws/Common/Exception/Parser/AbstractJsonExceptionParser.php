 ...
 */
abstract class AbstractJsonExceptionParser implements \WP_Cloud_Search\Aws\Common\Exception\Parser\ExceptionParserInterface
{
    /**
     * {@inheritdoc}
     */
    public function parse(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Guzzle\Http\Message\Response $response)
    {
        ...
        if (null !== ($json = \json_decode($response->getBody(\true), \true))) {
     ...
     * @return array
     */
    protected abstract function doParse(array $data, \WP_Cloud_Search\Guzzle\Http\Message\Response $response);
}

 ...
 */
namespace WP_Cloud_Search\Aws\Common\Iterator;

use WP_Cloud_Search\Aws\Common\Enum\UaString as Ua;
use WP_Cloud_Search\Aws\Common\Exception\RuntimeException;
use WP_Cloud_Search\Guzzle\Service\Resource\Model;
use WP_Cloud_Search\Guzzle\Service\Resource\ResourceIterator;
/**
 * Iterate over a client command
 */
class AwsResourceIterator extends \WP_Cloud_Search\Guzzle\Service\Resource\ResourceIterator
{
    /**
    ...
    protected function sendRequest()
    {
        do {
            ...
            $this->command->add(\WP_Cloud_Search\Aws\Common\Enum\UaString::OPTION, \WP_Cloud_Search\Aws\Common\Enum\UaString::ITERATOR);
    ...
    protected function handleResults(\WP_Cloud_Search\Guzzle\Service\Resource\Model $result)
    {
        $results = array();
        ...
        if ($tokenParam = $this->get('input_token')) {
            ...
            if (\is_array($tokenParam)) {
                if (\is_array($this->nextToken) && \count($tokenParam) === \count($this->nextToken)) {
                    ...
                    throw new \WP_Cloud_Search\Aws\Common\Exception\RuntimeException('The definition of the iterator\'s token parameter and the actual token ' . 'value are not compatible.');
                }
            } else {
    ...
    protected function determineNextToken(\WP_Cloud_Search\Guzzle\Service\Resource\Model $result)
    {
        $this->nextToken = null;
     ...
     * @return mixed|null
     */
    protected function getValueFromResult(\WP_Cloud_Search\Guzzle\Service\Resource\Model $result, $key)
    {
        // Special handling for keys that need to access n-1 indexes
        if (\strpos($key, '#') !== \false) {
            $keyParts = \explode('#', $key, 2);
            $items = $result->getPath(\trim($keyParts[0], '/'));

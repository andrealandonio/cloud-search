 ...
 */
class WaiterConfigFactory implements \WP_Cloud_Search\Aws\Common\Waiter\WaiterFactoryInterface
{
    /**
     ...
     */
    public function __construct(array $config, \WP_Cloud_Search\Guzzle\Inflection\InflectorInterface $inflector = null)
    {
        $this->config = $config;
    ...
    {
        return new \WP_Cloud_Search\Aws\Common\Waiter\ConfigResourceWaiter($this->getWaiterConfig($waiter));
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function getWaiterConfig($name)
    {
        if (!$this->canBuild($name)) {
            throw new \WP_Cloud_Search\Aws\Common\Exception\InvalidArgumentException('No waiter found matching "' . $name . '"');
        }
        // inflect the name if needed
        $name = isset($this->config[$name]) ? $name : $this->inflector->camel($name);
        $waiter = new \WP_Cloud_Search\Aws\Common\Waiter\WaiterConfig($this->config[$name]);
        $waiter['name'] = $name;
        // Always use __default__ as the basis if it's set
        if (isset($this->config['__default__'])) {
            $parentWaiter = new \WP_Cloud_Search\Aws\Common\Waiter\WaiterConfig($this->config['__default__']);

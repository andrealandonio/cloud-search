 ...
 */
class CacheableCredentials extends \WP_Cloud_Search\Aws\Common\Credentials\AbstractRefreshableCredentials
{
    /**
     ...
     */
    public function __construct(\WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials, \WP_Cloud_Search\Guzzle\Cache\CacheAdapterInterface $cache, $cacheKey)
    {
        $this->cache = $cache;
    ...
    protected function refresh()
    {
        if (!($cache = $this->cache->fetch($this->cacheKey))) {
            ...
            if (!$this->credentials->isExpired()) {
                ...
                $this->cache->save($this->cacheKey, $this->credentials, $this->credentials->getExpiration() - \time());
            }
        } else {

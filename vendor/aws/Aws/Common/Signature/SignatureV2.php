 ...
 */
class SignatureV2 extends \WP_Cloud_Search\Aws\Common\Signature\AbstractSignature
{
    public function signRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        ...
        $timestamp = $this->getTimestamp(\true);
        ...
        $path = '/' . \ltrim($request->getUrl(\true)->normalizePath()->getPath(), '/');
        ...
        $signature = \base64_encode(\hash_hmac('sha256', $sign, $credentials->getSecretKey(), \true));
     ...
     */
    public function addParameter(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, $key, $value)
    {
     ...
     * @return string
     */
    private function getCanonicalizedParameterString(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() == 'POST') {

 ...
 */
class SignatureV3Https extends \WP_Cloud_Search\Aws\Common\Signature\AbstractSignature
{
    public function signRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        ...
        if (!$request->hasHeader('date') && !$request->hasHeader('x-amz-date')) {
            $request->setHeader('Date', \gmdate(\WP_Cloud_Search\Aws\Common\Enum\DateFormat::RFC1123, $this->getTimestamp()));
        }
        ...
        $signature = \base64_encode(\hash_hmac('sha256', $stringToSign, $credentials->getSecretKey(), \true));

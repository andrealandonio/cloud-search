 ...
 */
namespace WP_Cloud_Search\Aws\Common\Signature;

use WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface;
use WP_Cloud_Search\Aws\Common\Enum\DateFormat;
use WP_Cloud_Search\Aws\Common\HostNameUtils;
...
use WP_Cloud_Search\Guzzle\Stream\Stream;
/**
 * Signature Version 4
 * @link http://docs.aws.amazon.com/general/latest/gr/signature-version-4.html
 */
class SignatureV4 extends \WP_Cloud_Search\Aws\Common\Signature\AbstractSignature implements \WP_Cloud_Search\Aws\Common\Signature\EndpointSignatureInterface
{
    ...
    public function signRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        $timestamp = $this->getTimestamp();
        $longDate = \gmdate(\WP_Cloud_Search\Aws\Common\Enum\DateFormat::ISO8601, $timestamp);
        ...
        if ($request->hasHeader('x-amz-date') || !$request->hasHeader('Date')) {
            ...
            $request->setHeader('Date', \gmdate(\WP_Cloud_Search\Aws\Common\Enum\DateFormat::RFC1123, $timestamp));
        }
        ...
        if (!$region || !$service) {
            $url = \WP_Cloud_Search\Guzzle\Http\Url::factory($request->getUrl());
            $region = $region ?: \WP_Cloud_Search\Aws\Common\HostNameUtils::parseRegionName($url);
            $service = $service ?: \WP_Cloud_Search\Aws\Common\HostNameUtils::parseServiceName($url);
        }
    ...
    public function createPresignedUrl(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials, $expires)
    {
        $request = $this->createPresignedRequest($request, $credentials);
        $query = $request->getQuery();
        $httpDate = \gmdate(\WP_Cloud_Search\Aws\Common\Enum\DateFormat::ISO8601, $this->getTimestamp());
     ...
     */
    public static function convertPostToGet(\WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $cloned = \WP_Cloud_Search\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
     ...
     * @return string
     */
    protected function getPayload(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface) {
            if ($request->getMethod() == 'POST' && \count($request->getPostFields())) {
                ...
                return \WP_Cloud_Search\Guzzle\Stream\Stream::getHash($request->getBody(), 'sha256');
            }
        }
     ...
     * @return string
     */
    protected function getPresignedPayload(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        return $this->getPayload($request);
    }
    protected function createCanonicalizedPath(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        // POST requests can be sent as GET requests instead by moving the
        // POST fields into the query string.
        if ($request instanceof \WP_Cloud_Search\Guzzle\Http\Message\EntityEnclosingRequestInterface && $request->getMethod() === 'POST' && \strpos($request->getHeader('Content-Type'), 'application/x-www-form-urlencoded') === 0) {
            $sr = \WP_Cloud_Search\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
            // Move POST fields to the query if they are present
            foreach ($request->getPostFields() as $name => $value) {
     ...
     *               - signed_headers
     */
    private function createSigningContext(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, $payload)
    {
        $signable = array('host' => \true, 'date' => \true, 'content-md5' => \true);
    ...
    private function getSigningKey($shortDate, $region, $service, $secretKey)
    {
        ...
        if (!isset($this->hashCache[$cacheKey])) {
            ...
            $this->hashCache[$cacheKey] = \hash_hmac('sha256', 'aws4_request', $serviceKey, \true);
        }
     ...
     * @return string
     */
    private function getCanonicalizedQueryString(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        foreach ($queryParams as $key => $values) {
            ...
            foreach ((array) $values as $value) {
                if ($value === \WP_Cloud_Search\Guzzle\Http\QueryString::BLANK) {
                    $value = '';
                }
    ...
    private function addQueryValues($scope, \WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request, \WP_Cloud_Search\Aws\Common\Credentials\CredentialsInterface $credentials, $expires)
    {
    ...
    private function moveHeadersToQuery(\WP_Cloud_Search\Guzzle\Http\Message\RequestInterface $request)
    {
        $query = $request->getQuery();
